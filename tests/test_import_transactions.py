...
import src.config
from src.import_transactions import import_transactions_csv_excel
...
def test_case_1_all_ok(mock_read_csv):
    data = {
        "id": ["1", "2"],
    ...
    result = import_transactions_csv_excel("file.csv")
...
def test_case_3_missing_required_columns(mock_read_csv):
    ...
    with pytest.raises(ValueError, match="В файле отсутствуют обязательные колонки: state"):
        import_transactions_csv_excel("file.csv")
...
def test_case_4_wrong_type_required(mock_read_csv):
    ...
    with pytest.raises(ValueError, match=re.escape(
            "Ошибка в колонке 'id' строки 0: invalid literal for int() with base 10: 'abc'")):
        import_transactions_csv_excel("file.csv")
...
def test_case_4_5_wrong_type_optional(mock_read_csv):
    ...
    mock_read_csv.return_value = pd.DataFrame(data)
    with pytest.raises(ValueError, match=re.escape(
            "Ошибка в колонке 'from' строки 0: invalid literal for int() with base 10: 'acc1'")):
        import_transactions_csv_excel("file.csv")
...
def test_import_transactions_missing_columns_autoadd_mode1(mock_read_csv, monkeypatch):
    ...
    mock_read_csv.return_value = pd.DataFrame(data)
    result = import_transactions_csv_excel("fake.csv")
    assert len(result) == 1
...
def test_case_8_unsupported_filetype():
    with pytest.raises(ValueError,
                       match="Неподдерживаемый тип файла: file.txt. Поддерживаются только .json, .csv и .xlsx."):
        import_transactions_csv_excel("file.txt")
